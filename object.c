...
void UpdatePlayerObject(PlayerObject *player, Object *collisionObject[], int collisionObjectCount) {
  ...
  if (player->velocity.x != 0){
    ...
    for (int i = 0; i < collisionObjectCount; i++) {
      ...
      if (SDL_HasIntersectionF(&collisionObj->frect, &player->object.frect)) {
        if (player->velocity.x < 0)
          player->object.frect.x = collisionObj->frect.x + collisionObj->frect.w;
        else
          player->object.frect.x = collisionObj->frect.x - player->object.frect.w;
      }
    }
  }
  ...
  if (player->velocity.y != 0){
    ...
    for (int i = 0; i < collisionObjectCount; i++) {
      ...
      if (SDL_HasIntersectionF(&collisionObj->frect, &player->object.frect)) {
        if (player->velocity.y < 0)
          player->object.frect.y = collisionObj->frect.y + collisionObj->frect.h;
        else
          player->object.frect.y = collisionObj->frect.y - player->object.frect.h;
      }
    }
  }
